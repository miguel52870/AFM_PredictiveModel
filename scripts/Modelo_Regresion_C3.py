"""
modelo_regresion_c3.py — Modelo de regresión para predicción de fase PFM
...
Output       : C3_prep[N+1]                         → imagen continua 80×80 [0,1]
...
OUTPUT_DIR   = BASE_DIR / 'resultados' / 'modelo_regresion_c3'
...
class RegressionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        c3_next = self._load_npy(get_file(idx_c3_prep, pos + 1))
...
class SSIMLoss(nn.Module):
    ...
    def forward(self, pred, target):
        kernel  = self.kernel.to(pred.device).expand(pred.shape[1], 1, -1, -1)
...
def main():
    ...
    criterion = MSESSIMLoss(alpha=ALPHA)
    ...
    print(f"\n{'─'*65}")
    print(f"Mejor val SSIM (C3) : {best_ssim:.4f}  (epoch {best_epoch})")
